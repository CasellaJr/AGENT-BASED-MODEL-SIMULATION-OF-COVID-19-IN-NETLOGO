...
turtles-own [
  ...
  awareness? ;;check if our agent knows to be sick (true or false)
...
to setup ;;this procedure called "setup" sets up the simulation for start
  ...
  ;;without virus, alive, not immune, without knowing their state of sickness,
  ;;with a person shape because they represent persons, and in random x-y position, and with a size of 2 to see better
  create-turtles numberAgents [ setxy random-xcor random-ycor
    set virus? false set dead? false set immune? false set awareness? false set duration 0 set shape "person" set size 2 ]

  ;;and we want that one of these turtles gets infected:
  ask one-of turtles [set virus? true]
  ;;we want our turtles to be yellow is they are infected, blue if not infected, green if immune, red if dead
...
;;Inside this go procedure I will call several functions, move, spread, getAwareness, recolor and recover
...
to go
  ...
  ;;check if agents know they are sick
  if all? turtles [virus?] = true[
    ask turtles [getAwareness]
  ]
  ask turtles [move]
  ask turtles [spread]
...
to move
  ;;our agent will move only if it not dead
  if dead? = false [
    ;;i want that if an agent is sick there are 2 cases: it knows that is sick,
    ;;and it will follow self-quarantine (basically agent does not move); the other case is that it does not
    ;;know that he is sick, so he will continue to move, spreading the virus. Notice that, also
    ;;in the first case, when the agent knows that is sick, it will be stopped, but other
    ;;turtles can touch him anyway (think for example case of a family in a single room and one sick)
    ;;so, if awareness is false, agent will move
    ;;ask turtles [getAwareness]
    if awareness? = false[
...
to getAwareness
  if awareness? = false [
  if random-float 1.0 > awarenessProbability [ ;;this is the chance to know sickness; you can regulate awarenessProbability with the slider
      ;;basically if this probability is equal to 0, agents will always know their state. So, the higher the probability,
      ;;less chance to know their state -> less chance to quarantine (stop moving). If this prob is equal to 1, basically
      ;;it is like a button that turn off the possibility of quarantine.
...
;;yellow = infected, blue = not infected, immune = green, dead = red
to recolor
...
The model is initialized with 720 people, of which only 1 is infected. People move randomly about the world in one of the following states: not infected (blue), infected (yellow), immune (green). People may die (red) of infection. 
...
The factors controlled by the four sliders interact to influence how likely the virus is to thrive in this population. Notice that in all cases, these factors must create a balance in which an adequate number of potential hosts remain available to the virus and in which the virus can adequately access those hosts.
